...
#nr = input("Get nr for loop count\n")
    try:
        nr = int(sys.argv[1])
    except: sys.exit("  %s loopcnt" % sys.argv[0])
    print "time for %d loops (ms)" % nr
    Tmaximorum = [0.0]
    for i in range(5):
        Tmin = 0.0 # Tmin & Tmax are computed on the fly
        Tmax = sys.float_info.max # 1.7976931348623157e+308
        Tarr = [] # List for Time max'm
        tsav = t = time.time()
        for _ in range(nr):
            tik += 1
            loop(tik)
            ...
            if Tmin < (time.time() - t):
                Tmin = time.time() - t
                if Tmin > max(Tmaximorum):
                   Tmaximorum.append(Tmin)
            elif Tmax > (time.time() - t):
                 Tmax = time.time() - t
            Tarr.append(Tmax)
            t = time.time()
#           Tmin = min(Tarr)
#           Tmax = max(Tarr)
        print "%3d Avg = %5.2f ms; Min_DT = %5.2f ms| Max_DT =\
%5.2f ms|" %\
            (i+1,round((time.time()-tsav)*1000.0/nr,2),\
            round(Tmax*1000.0,2),\
            round(Tmin*1000.0,2))
    print "Tmaximorum =%6.2f ms" % ((max(Tmaximorum))*1000)
